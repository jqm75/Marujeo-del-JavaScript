...
console.log(miArrego[1][2]); // 6
console.log(miArrego[2][0]); // 7
```
...
```
**MAP** → **Retorna un nuevo array** con los resultados de aplicar la función que se pasa como parámetro a cada elemento del array.
```
...
```
Si lo declaramos en la función, realiza acciones dentro del array. Y los descompone el array en sus elementos individuales y asignarlos en orden a los parámetros. Aplicado asó este operador se llama **Spread**
```
const numeros = [1, 2, 3];
...
Un **clousure** es una función autoejecutable que encpasula un serie de variables y deficiones locales que únicamente serán accesible si son devueltas con el operador `return`. Esto permite tener, en cierto modo, *variables privadas*. O mejor dicho, menos accesibles desde fuera. Se usa cada vez menos desde la llegada de ES6 (ECMAScript v6)
...
const getDatos = () => {
  return new Promise ((resolve, reject) => {
      setTimeout (() => {
        resolve (datos)
  }, 1500) // Tiempo de retardo que le damos a la funcion con setTimeout para simular un retraso de tiempo.
  })
}
